...
DEFAULT_NOTES = [
    "1. 计费基准：基础价 = 打印张数 × 单价（元/张）。单面每页计 1 张；双面每 2 页计 1 张（不足 2 页按 1 张）；打印份数按倍数累计。"
    "各介质覆盖率上限：普通打印纸 10%、不干胶纸 10%、高厚白卡纸 35%、双面铜版纸 45%、相纸不设上限、自带纸张 10%。",
    "2. 超标加收（分段阶梯累进递减）：超出覆盖率上限的覆盖量按覆盖率区间分段计费——「上限 ~ 25%」段加收 0.60 倍，"
    "「25% ~ 45%」段加收 0.40 倍，「45% 以上」段加收 0.20 倍。"
    "加收金额 =（各档超出量 × 对应系数之和）÷ 覆盖折算基准 × 单价。",
    "3. 覆盖率定义：以满版 A4 纯黑为 100%，按页面渲染像素明度逐页统计；文档平均覆盖率 = 总覆盖量 ÷ 打印面数。"
    "覆盖率计算 DPI 默认 300（可在程序中调整）。",
    "4. 双面打印说明：A4 普通复印纸支持硬件自动双面；高厚白卡纸与双面铜版纸支持手动双面打印；不干胶标签纸及相纸仅提供单面打印。",
    "5. 自带纸张说明：若使用自备纸张，需经确认设备兼容性后上机，按单面 0.20 元/张、双面 0.30 元/张收取纯耗墨服务费"
    "（同样适用上述覆盖率上限与超标加收规则）。",
    "6. 设备服务说明：当前所有文印作业由 Brother DCP-T725DW 彩色喷墨多功能一体机提供，激光打印服务目前处于暂缓开放状态。",
    "7. 结算说明：应收合计 = 基础价 + 超标加收；打印完成后按实收金额结算，优惠 = 应收合计 − 实收金额（实收高于应收时为负数）。",
]
